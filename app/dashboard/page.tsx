...
// Layout: youtube-background (fixed z-0) + page-wrapper (relative z-10, normal flow)
// NO position:fixed on the content — that was collapsing mobile
...
function analyzePersonality(responses: Record<number, any>) {
  const text = Object.values(responses).map((v: any) => Array.isArray(v) ? v.join(' ') : String(v)).join(' ').toLowerCase()
  ...
  Object.entries(kwMap).forEach(([id, kws]) => {
    ...
    if (m > 0) weights[id] = Math.min(0.9, m * 0.18)
  })
  const sel = (Array.isArray(responses[2]) ? responses[2].join(' ') : String(responses[2] || '')).toLowerCase()
  Object.entries(kwMap).forEach(([id, kws]) => { if (kws.some(k => sel.includes(k))) weights[id] = 0.9 })
  return {
    preferredGenres: weights,
    personalityType: ['Intellectual Explorer','Emotional Connector','Entertainment Seeker','Escapist Explorer','Critical Analyst','Nostalgic Dreamer'][Math.floor(Math.random() * 6)],
  }
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem('onboardingCompleted')) { router.push('/onboarding'); return }

    setPageReady(true)
    const email = session?.user?.email || ''
    if (email) setFavGenres(persistentStorage.getFavoriteCategories(email))

    // Load existing ratings into map
    const stored = persistentStorage.getRatings(email || 'demo@user.com')
    const map: Record<string, number> = {}
    stored.forEach((r: any) => { if (r.movieId) map[String(r.movieId)] = r.rating })
    setUserRatings(map)

    loadRecs()

    // Register with friends API
    fetch('/api/friends', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'register', email, name: session?.user?.name, username: (session?.user as any)?.username }) }).catch(() => {})
  ...
  const loadRecs = async (boostGenres?: string[]) => {
    ...
    try {
      ...
      if (boostGenres?.length) boostGenres.forEach(id => { profile.preferredGenres[id] = 0.95 })
      ...
      if (data.success && data.recommendations.length > 0) {
        ...
        setRecStats({ accuracy: Math.min(99, 88 + Object.keys(profile.preferredGenres).length * 2), count: data.recommendations.length })
      }
  ...
  const doSearch = async (q: string) => {
    if (q.length < 2) { setShowSearch(false); setSearchResults([]); return }
    setIsSearching(true)
    try {
  ...
  const debouncedSearch = useCallback(debounce(doSearch, 420), []) // eslint-disable-line
  ...
  const addToWatchlist = async (movie: any) => {
    ...
    if (ok) {
      fetch('/api/friends', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'activity', userEmail: email, name: session?.user?.name,
          verb: 'added', title: movie.title || movie.name, mediaType: movie.media_type === 'tv' ? 'tv' : 'movie', posterPath: movie.poster_path }) }).catch(() => {})
      fetch('/api/watchlist', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ movieId: movie.id, title: movie.title || movie.name, poster_path: movie.poster_path,
          vote_average: movie.vote_average, release_date: movie.release_date || movie.first_air_date,
          overview: movie.overview, media_type: movie.media_type }) }).catch(() => {})
    }
  }
  ...
  const addPersonFav = (person: any) => {
    ...
    if (ok) { setShowSearch(false); setSearchQuery('') }
  }

  const rateMovie = async (movie: any, rating: number, review: string) => {
    const email = session?.user?.email || 'demo@user.com'
    const item  = { movieId: movie.id, title: movie.title || movie.name || movie.displayTitle || 'Unknown',
      poster_path: movie.poster_path, vote_average: movie.vote_average,
      release_date: movie.release_date || movie.first_air_date, rating, review, media_type: movie.media_type || 'movie' }
    ...
    if (ok) {
      ...
      fetch('/api/movies/rate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(item) }).catch(() => {})
    }
  }
  ...
  const toggleGenre = (id: string) => {
    ...
    toast(added ? `${g?.emoji} ${g?.name} pinned — refreshing…` : `${g?.name} unpinned`, added ? 'ok' : 'info')
    if (added) loadRecs(updated)
  }

  const toast = (msg: string, type: 'ok' | 'info') => {
    const el   = document.createElement('div')
    el.style.cssText = `position:fixed;top:16px;right:16px;z-index:9999;padding:10px 16px;border-radius:10px;font-size:13px;font-family:'DM Sans',sans-serif;font-weight:500;color:#fff;pointer-events:none;transition:opacity 0.3s;${type === 'ok' ? 'background:linear-gradient(135deg,#22c55e,#16a34a);' : 'background:rgba(255,255,255,0.12);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.15);'}`
    el.textContent = msg
    document.body.appendChild(el)
    setTimeout(() => { el.style.opacity = '0'; setTimeout(() => el.remove(), 320) }, 2600)
  }

  // Username modal helpers
  const openUnModal = async () => {
    setShowUserMenu(false); setNewUsername((session?.user as any)?.username || '')
    setUnStatus(null); setShowUsernameModal(true)
  ...
  const handleUnInput = (val: string) => {
    setNewUsername(val); setUnStatus(null)
  ...
  const saveUn = async () => {
    ...
    setSavingUn(true)
    try {
      const res  = await fetch('/api/auth/change-username', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: newUsername }) })
      const data = await res.json()
      if (data.success) {
        setUnQuota(data.quota); toast(`Username → @${data.username}`, 'ok')
        setShowUsernameModal(false); await updateSession({ username: data.username })
      } else { setUnStatus({ error: data.error }) }
    } finally { setSavingUn(false) }
  }

  // ── Render guards ─────────────────────────────────────────────────────────
  if (status === 'loading' || !pageReady) return (
  ...
  return (
    <>
      {/* ── YouTube background: fixed behind everything ── */}
      ...
      {/* ── Tour (auto on first visit, manually replayable) ── */}
      <OnboardingTour key={tourKey} onReady={() => setTourReady(true)} />
            ...
            <motion.div initial={{ y: 20, scale: 0.97 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20, scale: 0.97 }}
              ...
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-white font-semibold flex items-center gap-2 text-sm">
                  <AtSign className="w-4 h-4 text-purple-400" />Change Username
                </h2>
                <button onClick={() => setShowUsernameModal(false)} className="text-white/30 hover:text-white/60 transition-colors"><X className="w-4 h-4" /></button>
              </div>
              {unQuota && (
                <div className="mb-4 px-3 py-2 rounded-lg text-xs text-white/45 flex gap-3" style={{ background: 'rgba(255,255,255,0.05)' }}>
                  <span>{unQuota.used}/{unQuota.max} changes</span>
                  <span className="w-px h-3 bg-white/15 self-center" />
                  {unQuota.daysLeft > 0 ? <span className="text-yellow-400">{unQuota.daysLeft}d cooldown</span> : <span className="text-green-400">Ready</span>}
                </div>
              )}
              <div className="relative mb-1">
                <AtSign className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/25" />
                <input type="text" value={newUsername} onChange={e => handleUnInput(e.target.value)}
              ...
              <div className="flex gap-2 mt-4">
                <button onClick={() => setShowUsernameModal(false)} className="flex-1 py-2.5 rounded-lg text-sm text-white/50 hover:text-white transition-colors" style={{ background: 'rgba(255,255,255,0.06)' }}>Cancel</button>
                <button onClick={saveUn} disabled={savingUn || !newUsername || newUsername.length < 3 || unStatus?.available === false || !unQuota?.canChange}
                  className="flex-1 py-2.5 rounded-lg text-sm font-semibold text-white transition-colors disabled:opacity-30"
                  style={{ background: '#7c3aed' }}>
                  {savingUn ? 'Saving…' : 'Save'}
                </button>
              </div>
            </motion.div>
          </motion.div>
      ...
      {/* ── PAGE WRAPPER: normal flow (NOT position:fixed) ── */}
        ...
        <header className="sticky top-0 z-30 px-4 py-3 md:px-6"
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2.5 flex-shrink-0">
              ...
              <span className="text-white font-bold text-base hidden sm:block" style={{ letterSpacing: '-0.03em' }}>CineScope</span>
              {userProfile && (
                <span className="hidden md:flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-lg text-purple-300"
                  style={{ background: 'rgba(139,92,246,0.14)', border: '1px solid rgba(139,92,246,0.18)' }}>
                  <Brain className="w-3 h-3" />{userProfile.personalityType}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-1.5 md:gap-2">
              {/* Tour replay — only show after tour has run once */}
              {tourReady && (
                <button onClick={() => { localStorage.removeItem('cinescope_tour_done_v1'); setTourKey(k => k + 1); setTourReady(false) }}
              ...
              {/* Refresh AI */}
              <button onClick={() => loadRecs(favGenres)} disabled={isLoadingRecs}
                ...
                <span className="hidden md:inline text-xs">{isLoadingRecs ? 'Thinking…' : 'Refresh AI'}</span>
              </button>

              {/* User menu */}
              <div className="relative">
                <button onClick={() => setShowUserMenu(!showUserMenu)} data-tour="user-menu"
                  ...
                  <span className="hidden md:inline text-xs font-medium">{session?.user?.name || 'User'}</span>
                </button>
                ...
                <AnimatePresence>
                  {showUserMenu && (
                    <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}
                      ...
                      className="absolute right-0 mt-2 w-48 rounded-xl py-1.5 shadow-2xl z-50"
                      ...
                      {[
                        ...
                        <button key={item.path} onClick={() => { setShowUserMenu(false); router.push(item.path) }}
                          className="w-full text-left px-4 py-2 text-white/75 hover:text-white hover:bg-white/8 transition-colors flex items-center gap-2.5 text-sm">
                          <item.icon className="w-3.5 h-3.5 text-white/35" />{item.label}
                        </button>
                      ))}
                      <div className="my-1 border-t border-white/8" />
                      <button onClick={openUnModal}
                        className="w-full text-left px-4 py-2 text-white/75 hover:text-white hover:bg-white/8 transition-colors flex items-center gap-2.5 text-sm">
                        <AtSign className="w-3.5 h-3.5 text-purple-400" />Change Username
                      </button>
                      <div className="my-1 border-t border-white/8" />
                      <button onClick={async () => { await signOut({ redirect: false }); router.push('/') }}
                        className="w-full text-left px-4 py-2 text-white/75 hover:text-white hover:bg-white/8 transition-colors flex items-center gap-2.5 text-sm">
                        <LogOut className="w-3.5 h-3.5 text-white/35" />Sign Out
                      </button>
                    </motion.div>
                  )}
                </AnimatePresence>
        ...
        <section className="px-4 pt-10 pb-6 md:px-6">
          <div className="max-w-7xl mx-auto">
            ...
            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              className="max-w-2xl mx-auto relative" data-tour="search-bar">
              <div className="relative">
                ...
                {isSearching
                  ? <div className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 border-2 border-yellow-400 border-t-transparent rounded-full animate-spin" />
                  : searchQuery && <button onClick={() => { setSearchQuery(''); setShowSearch(false) }} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/60"><X className="w-4 h-4" /></button>
                }
              </div>
              ...
              <AnimatePresence>
                {showSearch && searchResults.length > 0 && (
                  <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    ...
                    style={{ background: 'rgba(10,8,22,0.98)', border: '1px solid rgba(255,255,255,0.1)' }}>
                    {searchResults.map(r => (
                      <div key={r.id} className="flex items-center gap-3 px-3 py-2.5 hover:bg-white/7 transition-colors cursor-pointer"
                        onClick={() => { if (r.media_type !== 'person') { setMovies(p => [r, ...p.slice(0,11)]); setShowSearch(false); setSearchQuery('') } }}>
                        <div className="w-8 h-11 rounded overflow-hidden flex-shrink-0 bg-white/8">
                          {(r.poster_path || r.profile_path)
                            ? <Image src={`https://image.tmdb.org/t/p/w92${r.poster_path || r.profile_path}`} alt={r.title || r.name} width={32} height={44} className="w-full h-full object-cover" />
                            : <div className="w-full h-full flex items-center justify-center text-xs">{r.media_type === 'person' ? '👤' : '🎬'}</div>}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-white text-sm font-medium truncate">{r.title || r.name}</p>
                          <p className="text-white/38 text-xs">{r.media_type === 'person' ? r.known_for_department || 'Actor' : r.media_type === 'tv' ? 'TV' : 'Film'}{r.release_date && ` · ${new Date(r.release_date).getFullYear()}`}</p>
                        </div>
                        <button onClick={e => { e.stopPropagation(); r.media_type === 'person' ? addPersonFav(r) : addToWatchlist(r) }}
        ...
        <section className="px-4 pb-5 md:px-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              {[...GENRES.filter(g => favGenres.includes(g.id)), ...GENRES.filter(g => !favGenres.includes(g.id))].map(g => {
