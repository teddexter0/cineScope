...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let emailExists: boolean | null = null // null = unknown (DB error)
      try {
        ...
        // FIXED: handle the new dbError case — treat as unknown so we
        // fall straight through to signIn() rather than blocking the user.
        if (data.dbError || data.exists === null) {
          emailExists = null
        } else {
          emailExists = data.exists
        }
      } catch {
        // Network error on the check — fall through to signIn()
        emailExists = null
      }

      // Only block if we are CERTAIN the email doesn't exist
      ...
      // Step 2: attempt sign-in (covers both confirmed-exists and unknown cases)
      ...
      if (result?.error) {
        // If we fell through because of a DB error and the password is also
        // wrong, this is the right place to show the generic error.
        if (emailExists === null) {
          setError('unknown')
        } else {
